...
public class VentanaJuego extends JFrame {
    ...
    public VentanaJuego(Jugador jugador1, Jugador jugador2, int modo, int numeroRondas, int score1, int score2) {
        grid = new ImageIcon("img/gridF.png");
...
//Para cuando el pc gane, entonces le pasa el turno al jugador (solo cuando es modo 2 )
    ...
    private void TerminarJuego() {
        ...
        if (gameOver == true && numeroRounds == 0) {
            ...
            respuesta = JOptionPane.showConfirmDialog(
                    ...
                    System.out.println("el numero d rondas fue" + numeroRounds);
            if (respuesta == JOptionPane.YES_OPTION) {
                ...
                VentanaEstadisticas ventanaEstadisticas = new VentanaEstadisticas(ganador,jugador1, jugador2, modoJuego, scorePlayer1, scorePlayer2);
                dispose();
                } else if (modoJuego == 2) {
                Jugador jugador1 = new Jugador(nombreJugador1);
                Jugador jugador2 = new Jugador("PC");
                VentanaEstadisticas ventanaEstadisticas = new VentanaEstadisticas(ganador,jugador1, jugador2, modoJuego, scorePlayer1, scorePlayer2);
                dispose();
            }
            }
    ...
    private void DibujarTablero() {
        ...
        for (int i = 0; i < tableroSize; i++) {
            ...
            for (int j = 0; j < tableroSize; j++) {                
                ...
                buttons[j][i].setIcon(new ImageIcon(grid.getImage().getScaledInstance(buttons[j][i].getWidth(),
                buttons[j][i].getHeight(), Image.SCALE_SMOOTH)));
                ...
                buttons[j][i].addMouseListener(new ManejadoraDeMouse());
    ...
    private void AltGrid(JButton casilla){
        ImageIcon altGrid = new ImageIcon("img/gridF2.png");
        casilla.setIcon(new ImageIcon(altGrid.getImage().getScaledInstance(
                casilla.getWidth(), casilla.getHeight(), Image.SCALE_SMOOTH)));
    }
    
    private void NormalGrid(JButton casilla){
        ImageIcon normalGrid = new ImageIcon("img/gridF.png");
        casilla.setIcon(new ImageIcon(normalGrid.getImage().getScaledInstance(
                casilla.getWidth(), casilla.getHeight(), Image.SCALE_SMOOTH)));
    }
    ...
    private void iniciarComponentesModo1() {
        ...
        btnTerminar.setBounds(550,390, 100,40);
